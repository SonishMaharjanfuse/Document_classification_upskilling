...
def vgg_predict():
  ...
  if st.button("Predict"):
    ...
    st.write("predict using VGG")


options = st.sidebar.radio("Choose the model", ["Home", "YOLO", "VGG"])
if options == "YOLO":
    yolo_options = st.sidebar.radio("Choose option", ["About model", "Prediction"])
...
The same model was used for docuent classification (citizenship, license, others and passport).For training purpose we used augmented dataset that we prepared in week 2. In comprasion to other model YOLO has performed extreamly well in our case.
"""
        )
...
if options == "VGG":
    vgg_options = st.sidebar.radio("Choose option", ["About model", "Prediction"])
